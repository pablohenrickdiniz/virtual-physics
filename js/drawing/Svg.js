function Svg(svg){
	if(document.getElementById(svg) == null){
		throw new TypeError('Elemento svg n�o existe');
	}
	this.svg = svg;
	this.width = toInt($("#"+svg).css('width'));
	this.height = toInt($("#"+svg).css('height'));
	
	this.desenharFormaGeometrica = function(formaGeometrica) {
		if (formaGeometrica instanceof Polygon) {
			this.desenharPoligono(formaGeometrica);
		} else if (formaGeometrica instanceof Circle) {
			this.desenharCirculo(formaGeometrica);
		}
		else if(formaGeometrica instanceof Line){
			this.desenharReta(formaGeometrica);
		}
	};

	this.desenharCirculo = function(circulo) {
		if (circulo instanceof Circle) {
			...
			$(circle).attr("cx", circulo.getCenter().getX());
			$(circle).attr("cy", circulo.getCenter().getY());
			$(circle).attr("r", circulo.getRadius());
	...
	this.desenharArco = function(arco) {
		if (arco instanceof Arc) {
			
		}
	};
	
	this.desenharReta = function(reta){
		if(reta instanceof Line){
	...
	this.desenharPoligono = function(poligono) {
		if(poligono instanceof Polygon) {
	...
	this.desenhar = function(desenho){
		if(desenho instanceof Drawing){
			var formas = desenho.getShapes();
	...
	this.preencherForma = function(forma) {
		var cor = forma.getColor();
		if (cor instanceof LinearGradient || cor instanceof RadialGradient) {
			return this.fillGradient(forma);
		} else {
			return this.preencherCor(cor);
		}
	};
	
	this.fillGradient = function(forma) {
		var gradiente = forma.getColor();
		var quad = forma.getCircumscribedSquare();
		
		var id = gradiente.getId();
		var gradienteElement = document.getElementById(id);
		if(gradienteElement == null){
			
			if (gradiente instanceof RadialGradient) {
	...
	this.preencherBorda = function(element, borda){
		$(element).attr("stroke", borda.getColor());
		$(element).attr("stroke-width", borda.getThickness());
